...
def safe_remove_parametrizations(module, tensor_name):
    try:
        remove_parametrizations(module, tensor_name)
    except ValueError:
        pass
...
class ResBlock1(torch.nn.Module):
    ...
    def remove_weight_norm(self):
        for l in self.convs1:
            safe_remove_parametrizations(l, "weight")
...
class HifiganGenerator(torch.nn.Module):
    def __init__(
        ...
        if not conv_pre_weight_norm:
            safe_remove_parametrizations(self.conv_pre, "weight")

        if not conv_post_weight_norm:
            safe_remove_parametrizations(self.conv_post, "weight")
